...
module.exports = {
    ...
    run: async (client, interaction, player) => {
        ...
        if (player && player.voiceId !== interaction.member.voice.channelId) {
            return interaction.editReply(`${e.ICON_WARNING} You must be in the same voice channel as the bot.`);
        }

        const query = interaction.options.getString("query");
        
        // 3. Update the deferred message so the user knows the bot is searching
        await interaction.editReply(`${e.ICON_SEARCH} Searching for \`${query}\``);
        ...
        if (result.type === "EMPTY" || result.type === "ERROR" || !result.tracks.length) {
            ...
            if (result.type === "ERROR") {
                return interaction.editReply(`${e.ICON_ERROR} Search error:${errorMessage || 'Unknown error occurred during search.'}`);
            } else if (result.type === "EMPTY") {
                return interaction.editReply(`${e.ICON_ERROR} No results found for your query. The video may be unavailable, restricted, or the search returned no results.`);
            } else if (!result.tracks.length) {
                return interaction.editReply(`${e.ICON_ERROR} Search completed but no tracks were found. Please TRY AGAIN`);
            } else {
                return interaction.editReply(`${e.ICON_ERROR} No results found for your query.`);
            }
        }
        ...
        if (result.type === "PLAYLIST") {
            ...
            if (availableSpace <= 0) {
                return interaction.editReply(`${e.ICON_WARNING} Queue is full (max${client.config.maxQueueSize} tracks). Cannot add playlist.`);
            }
            ...
            if (tracksAdded === 0) {
                return interaction.editReply(`${e.ICON_WARNING} Queue is full (max${client.config.maxQueueSize} tracks). Cannot add any tracks from this playlist.`);
            ...
            if (currentSize >= client.config.maxQueueSize) {
                return interaction.editReply(`${e.ICON_WARNING} Queue is full (max${client.config.maxQueueSize} tracks). Cannot add this track.`);
            }
