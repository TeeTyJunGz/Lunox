...
const SPOTIFY_RETRY_COUNT = 5;    // number of retries after first failure
const SPOTIFY_RETRY_DELAY_MS = 2500; // ms to wait between each retry
...
module.exports = {
    ...
    run: async (client, interaction, player) => {
        ...
        if (player && player.voiceId !== interaction.member.voice.channelId) {
            return interaction.editReply(`${e.system.warning} You must be in the same voice channel as the bot.`);
        }

        const query = interaction.options.getString("query");
        
        // 3. Update the deferred message so the user knows the bot is searching
        await interaction.editReply(`${e.system.search} Searching for \`${query}\``);
        ...
        if (result.type === "EMPTY" || result.type === "ERROR" || !result.tracks.length) {
            ...
            if (result.type === "ERROR" && errorMessage) {
                Logger.error(`[PLAY COMMAND] Search error for query "${query}":`, errorMessage);
            }

            if (isSpotifyAutoGeneratedPlaylistError(errorMessage) || isSpotifyAutoGeneratedPlaylistUrl(query)) {
                return interaction.editReply(
                    `${e.system.warning} **Spotify auto-generated playlists (Daily Mix, Discover Weekly, Release Radar, radio stations, etc.) sometimes fail to load due to a Spotify-side restriction.**\n\nThe playlist you tried to play couldn't be loaded this time. You can try again — it may work on another attempt — or copy the songs into your own playlist on Spotify first for reliable playback.`
                );
            }

            if (result.type === "ERROR") {
                return interaction.editReply(`${e.system.error} Search error:${errorMessage || 'Unknown error occurred during search.'}`);
            } else if (result.type === "EMPTY") {
                return interaction.editReply(`${e.system.error} No results found for your query. The video may be unavailable, restricted, or the search returned no results.`);
            } else if (!result.tracks.length) {
                return interaction.editReply(`${e.system.error} Search completed but no tracks were found. Please TRY AGAIN`);
            } else {
                return interaction.editReply(`${e.system.error} No results found for your query.`);
            }
        }
        ...
        if (result.type === "PLAYLIST") {
            ...
            if (availableSpace <= 0) {
                return interaction.editReply(`${e.system.warning} Queue is full (max${client.config.maxQueueSize} tracks). Cannot add playlist.`);
            }
            ...
            if (tracksAdded === 0) {
                return interaction.editReply(`${e.system.warning} Queue is full (max${client.config.maxQueueSize} tracks). Cannot add any tracks from this playlist.`);
            } else if (tracksAdded < result.tracks.length) {
                finalMessage = `${e.system.playlist} Added **${tracksAdded}** of **${result.tracks.length}** songs from **${result.playlistName}**. Queue limit reached.`;
            } else {
                finalMessage = `${e.system.playlist} Added **${result.playlistName}** - \`${result.tracks.length}\` songs to the queue.`;
            }
        } else {
            // Single track
            const currentSize = player.queue.size + (player.playing ? 1 : 0);
            if (currentSize >= client.config.maxQueueSize) {
                return interaction.editReply(`${e.system.warning} Queue is full (max${client.config.maxQueueSize} tracks). Cannot add this track.`);
            }
            ...
            finalMessage = `${e.buttons.play} Added [${trackTitle}](<${track.uri}>) — ${trackAuthor}  \`${convertTime(track.duration)}\``;
        }
...
async function refreshSpotifyTokenBackground() {
    ...
    try {
        Logger.debug(`[Spotify] Token missing/expired. Fetching new one in background...`);
        ...
        if (res.ok) {
            ...
            if (data.access_token) {
                ...
                Logger.info(`[Spotify] Background fetch complete. New token active.`);
            }
        }
    } catch (e) {
        Logger.debug(`[Spotify] Background token fetch failed:`, e.message);
    } finally {
...
async function rotateSpotifyTokenBackground() {
    ...
    isFetchingToken = true;
    try {
        Logger.debug(`[Spotify] 429 Rate Limit Hit! Triggering burner account rotation in background...`);
        ...
        if (res.ok) {
            ...
            if (data.access_token) {
                cachedSpotifyToken = data.access_token;
                tokenExpiresAt = Date.now() + (50 * 60 * 1000);
                Logger.info(`[Spotify] Rotation successful. Swapped to [${data.account}].`);
            }
        }
    } catch (e) {
        Logger.debug(`[Spotify] Background rotation failed:`, e.message);
    } finally {
        isFetchingToken = false;
    }
}
